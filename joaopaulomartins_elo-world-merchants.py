...
def reduce_mem_usage(df, verbose=True):
    ...
    return df
def agg_by_categoryid(merchants):
    agg_func = {
        'merchant_id': ['nunique'],
...
merchant_category.head(1)
merchant_category = merchant_category.rename(columns = {'merchant_category_id': 'hist_merchant_category_id_nunique'})
merchant_category.head(2)
train = pd.merge(train, merchant_category, on='hist_merchant_category_id_nunique', how = 'left')
test = pd.merge(test, merchant_category, on='hist_merchant_category_id_nunique', how = 'left')
test.head()
train = train.drop(['merchant_id_nunique', 'subsector_id_nunique'], axis =1)
test = test.drop(['merchant_id_nunique', 'subsector_id_nunique'], axis =1)
