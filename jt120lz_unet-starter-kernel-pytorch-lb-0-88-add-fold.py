    ...
    def start(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            if val_loss < self.best_loss:
                ...
                torch.save(state, f"./model_{self.fold}.pth")
...
fold = model_selection.StratifiedKFold(n_splits=5, shuffle=False)



for index, (train_idx, val_idx) in enumerate(fold.split(df, df['defects'])):

    train = df.iloc[train_idx].head()

    val = df.iloc[val_idx].head()
