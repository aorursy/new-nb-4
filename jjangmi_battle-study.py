...
solos = train[train['matchType'] == 'solo']
duos = train[train['matchType'] == 'duo']
squads = train[train['matchType'] == 'squad']
f,ax1 = plt.subplots(figsize =(20,10))
...
totalDistance = train['walkDistance']+train['rideDistance']+train['swimDistance']
train['zombi'] = ((train['totalDistance'] == 0) & (train['kills'] == 0)
                     & (train['weaponsAcquired'] == 0) 
                     & (train['matchType'].str.contains('solo'))).astype(int)
train['cheater'] = ((train['kills'] / train['totalDistance'] >= 1)
                       | (train['kills'] > 40) | (train['roadKills'] > 10)).astype(int)
del totalDistance
pd.concat([train['zombi'].value_counts(), train['cheater'].value_counts()], axis=1).T
train.head()