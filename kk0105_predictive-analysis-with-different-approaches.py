...
train_flattened.head()
count_df = pd.DataFrame(train_flattened[['Page','date','Visits']].groupby(['date'])['Visits'].count()).reset_index()



fig = go.Figure()

fig.add_trace(go.Scatter(x = count_df.date ,y = count_df.Visits,mode ='lines'))

fig.update_layout(template='seaborn',title=' Date visits counts')
