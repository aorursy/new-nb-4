...
TRAINING_FILENAMES = TRAINING_FILENAMES + VALIDATION_FILENAMES + ADDITIONAL_TRAINING_FILENAMES
...
lr_schedule = tf.keras.callbacks.LearningRateScheduler(lrfn1, verbose=True)
...
with strategy.scope():
    ...
    model.summary()
history = model.fit(get_training_dataset(), steps_per_epoch=STEPS_PER_EPOCH, epochs=20, callbacks=[lr_callback],validation_data=get_validation_dataset())
...
np.save('ensemble_1.npy',probabilities)
