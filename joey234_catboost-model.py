...
train[train['game_session']=='e7e7db2a241eadcc']
train_labels[train_labels['game_session'] == '0006a69f']
...
train.groupby('installation_id').head()
...
user_sample
len(train.groupby('installation_id'))
train_labels
...
# for i,(ins_id,user) in enumerate(train.groupby('installation_id', sort = False)):

#     if cond:

#         break

#     for j,(session_id,session) in enumerate(user.groupby('game_session',sort = False)):

# #         if j==1:

# #             break

#         if len(session) == 1 and session['type'].iloc[0] == 'Assessment':
...
#             if session['event_data'].iloc[0] in [4100,4110]:

#                 print('event data',session['event_data'].iloc[0])
...
#     regressor.fit(X.loc[trn_idx, all_features], y.loc[trn_idx], eval_set=(X.loc[test_idx, all_features], y.loc[test_idx]),
