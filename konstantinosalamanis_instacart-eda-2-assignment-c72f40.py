...
aisle_top_sort = aisle_top_sort.iloc[ :10]
...
avg_pos = order_products_prior.groupby('order_id').filter(lambda x: x.product_id.count() > 30)
...
plt.xlabel('Avg_pos_asc', size=10)
plt.ylabel('Product_id', size=10)
plt.ylim(0,2)
plt.show()