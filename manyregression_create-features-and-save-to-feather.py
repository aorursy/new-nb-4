...
test_df = pd.DataFrame(columns=train_X.columns, dtype=np.float64, index=submission.index)
for seg_id in tqdm(test_df.index):

    seg = pd.read_csv(path/f'test/{seg_id}.csv')

    create_features(seg_id, seg, test_df)
test_df.reset_index(inplace=True); test_df.shape
df = pd.concat([train_X, train_y], axis=1); df.shape
df.to_feather("df")

test_df.to_hdf('test_df.h5', key="test_df")
df.head(5)
test_df.head(5)
