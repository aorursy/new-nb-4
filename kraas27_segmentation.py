...
train_img_paths = sorted(glob('../input/train/*.jpg'))[:SAMPLE]

train_mask_paths = sorted(glob('../input/train_masks/*.gif'))[:SAMPLE]



train_imgs = np.array([cv2.resize(plt.imread(path), (IMG_ROWS, IMG_COLS)) for path in train_img_paths])



train_masks = np.array([cv2.resize(plt.imread(path), (IMG_ROWS, IMG_COLS)) for path in train_mask_paths], dtype="float32")
train_masks_2 = []

for num, i in enumerate(train_masks):

    train_masks_2.append(cv2.cvtColor(i, cv2.COLOR_BGR2GRAY))
train_masks = np.array(train_masks_2)
...
test_paths = sorted(glob('../input/test/*.jpg'))
...
def test_img_generator(test_paths):
    ...
    while True:
        ...
        for path in test_paths:
            ...
            yield np.array([cv2.resize(plt.imread(path), (IMG_ROWS, IMG_COLS))])
...
plt.imshow(plt.imread(test_paths[k]))
...
plt.imshow(plt.imread(test_paths[k+1]))
