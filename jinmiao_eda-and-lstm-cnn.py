...
import os
print(os.listdir("../input/embeddings/glove.840B.300d/")) # 使用的是Glove_840B_300d，那麼EMBEDDING_DIM=300
...
tk.fit_on_texts(full_text) # 要用以训练的文本列表
full_text[:3], len(full_text)
train.question_text.describe()
# 输入：待转为序列的文本列表
# 返回值：序列的列表，列表中每个序列对应于一段输入文本
train_tokenized = tk.texts_to_sequences(train['question_text'].fillna('missing'))
test_tokenized = tk.texts_to_sequences(test['question_text'].fillna('missing')) # tk.texts_to_sequences方法会丢失train中未出现过的单词
'missing' in test_tokenized
...
embedding_path = "../input/embeddings/glove.840B.300d/glove.840B.300d.txt"  # glove词向量
...
def build_model4(lr=0.0, lr_d=0.0, units=0, spatial_dr=0.0, kernel_size1=3, kernel_size2=2, dense_units=128, dr=0.1, conv_size=32, epochs=20):
    ...
    return model
###%%time
###model4 = build_model4(lr = 1e-4, lr_d = 1e-7, units = 64, spatial_dr = 0.3, kernel_size1=3, dense_units=32, dr=0.1, conv_size=8, epochs=5)
...
def build_model5(lr=0.0, lr_d=0.0, units=0, spatial_dr=0.0, kernel_size1=3, kernel_size2=2, dense_units=128, dr=0.1, conv_size=32, epochs=20):
    ...
    model = load_model(file_path)
    return model
###model5 = build_model5(lr = 1e-4, lr_d = 1e-7, units = 128, spatial_dr = 0.3, kernel_size1=4, dense_units=32, dr=0.1, conv_size=8, epochs=10)
...
sub['prediction'] = predictions
sub.to_csv("submission.csv", index=False)
sub.to_csv("submission_tag.csv", index=False)
